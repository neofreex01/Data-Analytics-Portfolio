...
This table suggests that institution quality and gdp have significantly positive relationship.

```{r}
model = lm(data = df4, lgdp ~ colony + tropics + inst )
...
stargazer(model1,model2, type = "latex",title="Table 4(d)", header = FALSE)

```

## (d)

The result in the folloing table shows IV estimation. The coefficient of inst ($\gamma$) is the same as we did in (c) and (d)

```{r}
model_2SLS = ivreg(data = df4, lgdp~colony+tropics+inst|col_trop+colony+tropics)
stargazer(model_2SLS,type = "latex", header = FALSE)
...
We find that the coefficient of $\gamma$ is the same as (e), but the variance is different. It's because when we are doing 2SLS separately in two stages, we misidentify the error term. Thus, we care about the variance in (e) more.

```{r}
fit_inst = fitted.values( model2)
df4 = cbind(df4, fit_inst)
model_sec = lm(data = df4, lgdp ~ colony + tropics + fit_inst)
stargazer(model2,model_sec, type = "latex", title = "Table 4(f)",header = FALSE)
```
...
stargazer(wald,summary = FALSE, type = "latex", covariate.labels = c("Wald estimate (Z1)","Wald estimate (Z2)"),rownames = FALSE, header = FALSE)
...
We reject the null hypothesis because the p-value of sargan test is 0.001 (In the result of Rmarkdown, it would be 0.0005)
It means only one of the instruments is valid, so it's overidentified.

```{r}

model_sargan = ivreg(data = df5, Y ~ `T` |Z1+Z2)
sargan = summary(model_sargan,diagnostic = TRUE)$diagnostics
stargazer(sargan, type="latex", header = FALSE, title = "sargan test")
...
stargazer(complier1,complier2,summary = FALSE, rownames = FALSE, type = "latex", header = FALSE)
...
for (i in 1:721){
  ...
  df5_test = df5 %>%
  mutate(Dep_var = if_else(Y<=test[i],1,0),untreated = 1-`T`,Dep_T = Dep_var*untreated) 
...
ggplot()+
  geom_point(aes(x = test,y = Z1_treated,color = "Z1:Y(1)"),size = 0.2, alpha = 0.2)+
  geom_point( aes(x = test,y = Z2_treated,color = "Z2:Y(1)"),size = 0.2,alpha = 0.2)+
  geom_point( aes(x = test,y = Z1_untreated,color = "Z1:Y(0)"),size = 0.2)+
  geom_point( aes(x = test,y = Z2_untreated,color = "Z2:Y(0)"),size = 0.2)+
  labs(x = "Y", y = "Probability", title = "Figure(5) (d)(iii)",
       caption  = "CDF of potential outcome for compliers with different instrument")+
  theme_classic()+
